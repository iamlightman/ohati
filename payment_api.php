...
function handle_payment_action($action, $pdo) {
    ...
    switch ($action) {
        case 'initiate_paystack_payment':
            ...
            // Paystack request
            $reference = 'OH-' . uniqid() . '-' . time();
            $email = $_SESSION['user']['email'] ?: 'customer@ohati.com';
            
            // In production, send to Paystack. For local demo/testing, we generate a mock authorization/payment link.
            $paystack_url = "https://checkout.paystack.com/mock-gateway?ref=" . $reference;
            
            // Insert escrow transaction
            $platform_fee = $amount * 0.10; // 10% platform fee
            $vendor_amount = $amount - $platform_fee;

            $stmt = $pdo->prepare("INSERT INTO escrow_transactions (booking_id, customer_id, vendor_id, amount, platform_fee, vendor_amount, paystack_reference, paystack_status, escrow_status) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', 'held')");
            ...
            audit_log($pdo, 'initiate_payment', 'escrow_transactions', $escrow_id, $amount, 'pending', 'held', "Initiated $payment_type payment for booking #$booking_id");
        ...
        case 'verify_paystack_payment':
            ...
            $escrow = $stmt->fetch();
            if (!$escrow) { http_response_code(404); echo json_encode(['error'=>'Transaction not found.']); exit; }

            if ($escrow['paystack_status'] === 'success') {
                echo json_encode(['success' => true, 'message' => 'Payment already verified.', 'escrow' => $escrow]);
                exit;
            }

            // In production, we'd verify with Paystack:
            // $paystack_res = verify_paystack_transaction($reference);
            // For testing/mock purposes, we auto-approve standard requests.
            ...
            if ($paystack_success) {
                ...
                try {
                    ...
                    $stmt = $pdo->prepare("UPDATE escrow_transactions SET paystack_status = 'success', escrow_status = 'held' WHERE id = ?");
                    ...
                    $stmt->execute([$escrow['booking_id'], $escrow['customer_id'], $escrow['vendor_id'], $escrow['amount'], $reference]);
                    ...
                    if ($new_total_paid >= $price_to_match) {
                        $payment_status = 'Paid';
                    }
                    ...
                    // Log activity
                    audit_log($pdo, 'payment_verified', 'escrow_transactions', $escrow['id'], $escrow['amount'], 'pending', 'held', "Paystack payment verified. Funds held in Escrow.");

                    // Notify customer and vendor
                    $stmt = $pdo->prepare("INSERT INTO notifications (user_id, title, body, icon) VALUES (?, 'Payment Successful', ?, 'circle-check')");
                    $stmt->execute([$escrow['customer_id'], "Your payment of GHS " . number_format($escrow['amount'], 2) . " for booking #" . $escrow['booking_id'] . " is secure in escrow."]);
                    ...
                    if ($vendor_user_id) {
                        $stmt = $pdo->prepare("INSERT INTO notifications (user_id, title, body, icon) VALUES (?, 'Booking Payment Received', ?, 'wallet')");
                        $stmt->execute([$vendor_user_id, "A payment of GHS " . number_format($escrow['vendor_amount'], 2) . " (after platform fee) has been received and is held in escrow."]);
                    }
                ...
                echo json_encode(['error' => 'Paystack transaction verification failed.']);
            }
            break;
        ...
        case 'admin_approve_withdrawal':
            ...
            // Perform Paystack transfer call here if connected. 
            // We mock verification.
            $paystack_success = true;
            ...
            try {
                if ($paystack_success) {
                    ...
                    audit_log($pdo, 'withdrawal_approved', 'withdrawals', $withdrawal_id, $w['amount'], 'pending', 'completed', "Withdrawal request approved and processed.");
