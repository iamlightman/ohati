...
if (file_exists(__DIR__ . '/ohati_config.php')) {
    ...
    $db_name = defined('DB_NAME') ? DB_NAME : 'ohati';
} else {
    $host = 'localhost';
    $db_user = 'root';
    $db_pass = '';
...
try {
    // Try connecting directly to MySQL database
    ...
    try {
        ...
        // Try local XAMPP MySQL fallback
        try {
            $pdo = new PDO("mysql:host=localhost;dbname=ohati;charset=utf8mb4", "root", "", [
                PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,
                PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC
            ]);
            $db_type = 'mysql';
        } catch (PDOException $eLocal) {
            // Local Development Fallback: Persistent SQLite Database File
            try {
                ...
                die(json_encode(['error' => 'Database connection failed: ' . $e3->getMessage()]));
            }
        }
    }
}
